...
def collate_fn(list_data):
    batched_pts_list, batched_gt_2d_pts_list, batched_gt_labels_list = [], [], []
    ...
    for data_dict in list_data:
        pts = np.array(data_dict['pts'])
        gt_2d_pts = np.array(data_dict['gt_2d_pts'])
        gt_labels = np.array(data_dict['gt_labels'])
        ...
        batched_gt_2d_pts_list.append(torch.from_numpy(gt_2d_pts))
        batched_gt_labels_list.append(torch.from_numpy(gt_labels))
    ...
    rt_data_dict = dict(
        ...
        batched_gt_2d_pts=batched_gt_2d_pts_list,
        batched_gt_labels=batched_gt_labels_list,
...
def get_dataloader_plr(dataset, batch_size, num_workers, shuffle=True, drop_last=False):
    collate = collate_fn
